...
class Client:
    ...
    def __init__(self,
        ...
        self.server_listener = SocketThread(self.client_udp,
                                            ...
                                            self.lock,
                                            self.client_udp)
        self.client_udp = (self.client_udp, self.server_listener.port)
...
class SocketThread(threading.Thread):
    def __init__(self, addr, client, lock, client_udp):
        """
        ...
        while self.port <= 65535:
            try:
                self.sock.bind((client_udp,self.port))
                break
            except OSError:
                self.port += 1
        if self.port > 65535:
